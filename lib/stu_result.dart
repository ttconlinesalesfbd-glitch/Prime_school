...
class _StudentResultPageState extends State<StudentResultPage> {
  ...
  Future<void> fetchExams() async {
    ...
    try {
      ...
      if (res == null) {
        if (mounted) setState(() => isExamLoading = false);
        return;
      }

      debugPrint("📦 EXAM STATUS: ${res.statusCode}");
      debugPrint("📦 RAW EXAM BODY: ${res.body}");

      if (res.statusCode != 200) {
        _showError("Failed to load exams (${res.statusCode})");
        return;
      }
      ...
      // ✅ CASE 2: API returns MAP (current backend behaviour)
      else if (decoded is Map) {
        examList = decoded.values
            .map((e) => Map<String, dynamic>.from(e))
            .toList();
      } else {
        _showError("Invalid exam data format");
        return;
      }
      ...
      _showError("Something went wrong while loading exams");
    }
  }
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final List marks = student['Marks'] is List
                        ? student['Marks']
                        : [];
